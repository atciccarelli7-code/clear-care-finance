...
- **Consequences:** The private-loan recommendation engine cannot import partner configuration; federal, mixed, and uncertain debt fails into verification; My Plan stores only a fixed action; financial assumptions remain user-controlled copy/print data; partner activation requires verified, current, disclosed configuration.
