...
- **Implementation:** Added a typed two-route hero-action registry, reused the fixed privacy-safe directional-action contract, promoted the nurse article into the priority hierarchy, centralized article tracking context, suppressed only the two redundant related-tool cards, and added route-unit and desktop/mobile Playwright regression coverage.
