...
- **Release state:** PR #265 on `agent/search-demand-product-buildout`; exact-head preview `dpl_2Xt7RW3FqUPEs3QNybtQXaXjMShR` is `READY`; deployed desktop/keyboard/metadata/source/redirect/runtime smoke passes. Release is blocked because GitHub instantiated no Actions run after opened/ready/synchronize/reopened events and the local Playwright fallback lacks Chromium; mobile/axe certification, merge, and production remain pending.
