...
- **Release state:** PR #265 on `agent/search-demand-product-buildout`; exact-head preview `dpl_CSoUuuBveHZThkMAagPTSBFrQ9oH` is `READY`; deployed desktop/keyboard/metadata/source/redirect/runtime smoke passes; repository browser CI, merge, and production identifiers remain release gates.
...
- **Unresolved warning:** Post-release search and completion data do not exist; source links and policies can change; provider exclusions and deadlines remain direct-verification risks; live Search Console query/page joins and URL Inspection were unavailable.
