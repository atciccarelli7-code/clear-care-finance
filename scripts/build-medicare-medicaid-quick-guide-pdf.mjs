...
const pageMeta = {
  1: {
    ...
    verify: "The problem, payer, rule, and document trail",
    risk: "Skipping the first question can send the family down the wrong path.",
  },
  2: {
    ...
    verify: "Whether coverage is approved, limited, pending, or denied",
    risk: "Assumptions can hide patient responsibility and appeal deadlines.",
  },
  3: {
    eyebrow: "Program map",
    ...
    verify: "Medicare, Medicaid, both, or neither",
    risk: "The wrong program assumption can create the wrong coverage expectation.",
  },
  4: {
    ...
    verify: "Original Medicare, Medicare Advantage, and any extra coverage",
    risk: "Network, authorization, and cost rules may change the next step.",
  },
  5: {
    ...
    verify: "Official hospital status and when inpatient admission began",
    risk: "Observation time can affect downstream Original Medicare SNF rules.",
  },
  6: {
    ...
    verify: "Destination, payer, approval status, and backup plan",
    risk: "Leaving without the payment path can create avoidable confusion later.",
  },
  7: {
    ...
    verify: "Skilled need, facility fit, prior authorization, and appeal window",
    risk: "Coverage can end or be denied faster than families expect.",
  },
  8: {
    ...
    verify: "Whether the need is skilled care, equipment, custodial help, or long-term support",
    risk: "Daily living help is often the coverage gap families discover late.",
  },
  9: {
    eyebrow: "Bill check",
    ...
    verify: "Bill, date of service, MSN/EOB, and patient responsibility",
    risk: "A bill can be premature, miscoded, duplicated, or misunderstood.",
  },
  10: {
    ...
    verify: "Who answered, what they said, what document supports it, and the deadline",
    risk: "Unwritten answers are hard to use when coverage or billing changes.",
  },
};
...
const renderVerificationBar = (pageNumber) => {
  const meta = pageMeta[pageNumber];
  if (!meta?.verify || !meta?.risk || pageNumber === 1) return "";

  return `<div class="verification-bar">
    <div><span>Verify</span><strong>${inline(meta.verify)}</strong></div>
    <div><span>Risk if missed</span><strong>${inline(meta.risk)}</strong></div>
  </div>`;
};
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    h1 { font-size: 21pt; line-height: 1.02; margin-bottom: 0.045in; letter-spacing: -0.035em; }
    ...
    .badge { height:.4in; width:.4in; border-radius: 15px; background: linear-gradient(135deg, var(--page-accent), var(--page-accent-2)); color:white; display:grid; place-items:center; font-weight:900; font-size:12.5pt; box-shadow: inset 0 -1px 0 rgba(0,0,0,.12); }
    ...
    .verification-bar { display:grid; grid-template-columns: 1fr 1fr; gap:.055in; break-inside: avoid; page-break-inside: avoid; }
    .verification-bar div { border:1px solid var(--line); border-radius:14px; background:#fff; padding:.055in .07in; box-shadow:var(--micro-shadow); }
    .verification-bar span { display:block; font-size:6.45pt; line-height:1; text-transform:uppercase; letter-spacing:.09em; color:var(--page-accent); font-weight:900; margin-bottom:.028in; }
    .verification-bar strong { display:block; font-size:7.45pt; line-height:1.13; color:var(--ink); }
    ...
    .takeaway-ribbon { display:grid; grid-template-columns:.74in 1fr; align-items:center; border:1.5px solid var(--page-accent); border-radius:15px; overflow:hidden; background:#fff; box-shadow:var(--micro-shadow); break-inside: avoid; page-break-inside: avoid; }
    .takeaway-ribbon span { align-self:stretch; display:grid; place-items:center; background:var(--page-accent); color:white; font-size:6.7pt; font-weight:900; letter-spacing:.08em; text-transform:uppercase; }
    .takeaway-ribbon strong { padding:.052in .074in; font-size:8.25pt; line-height:1.16; color:var(--ink); }
    ...
    .cover-step { position:relative; border:1px solid var(--line); border-radius:15px; background:white; padding:.064in .052in; min-height:.68in; box-shadow:var(--micro-shadow); }
    .cover-step:not(:last-child)::after { content:""; position:absolute; right:-.037in; top:.31in; width:.073in; height:.073in; border-top:2px solid var(--page-accent); border-right:2px solid var(--page-accent); transform:rotate(45deg); }
    ...
    .flow-card { position:relative; display:grid; break-inside: avoid; page-break-inside: avoid; }
    .flow-card:not(:last-child)::after { content:""; position:absolute; right:-.036in; top:.27in; width:.076in; height:.076in; border-top:2px solid var(--page-accent); border-right:2px solid var(--page-accent); transform:rotate(45deg); }
    .flow-card .mini-card { grid-template-columns:1fr; text-align:center; padding:.056in .04in; min-height:.6in; }
    .flow-card .mini-icon { margin:0 auto .028in; background:var(--page-accent); color:white; border-color:var(--page-accent); }
    ...
    .warning-grid .mini-copy div:last-child,.flow-grid .mini-copy div:last-child { font-size:7.15pt; }
    ...
    @media print {
      .cover-hero,.answer-panel,.visual-section,.mini-card,.decision-pill,.page-title,.cover-chip,.takeaway-ribbon,.cover-step,.verification-bar div { box-shadow:none; }
