...
const sourceMetadata = (context: EmployerBenefitsSourceContext) => {
  ...
  if (!source) return "No public source link was attached. Enter and verify all figures manually.";
  return [source.planYearLabel, source.audience, source.stateRegion].filter(Boolean).join(" · ")
    || "Current external employer source";
};
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-muted-foreground">
            CAF is independent from this employer and the source owner. The link is attached for reference only; CAF does not host or reproduce the document and has not copied premiums, deductibles, retirement formulas, eligibility rules, or other figures into this workspace. Confirm the controlling plan documents, effective dates, location, and employee group before entering values.
          </p>
        </div>
        <div className="flex shrink-0 flex-wrap gap-2">
          {context.selectedSource && (
            <Button asChild size="sm" variant="outline">
              <a href={context.selectedSource.url} target="_blank" rel="noreferrer noopener external">
          ...
          >
            Remove link <X className="h-4 w-4" aria-hidden="true" />
          </Button>
        </div>
      </div>
    </div>
