...
const ARTICLES_WITH_REVIEW_METADATA = applySearchOpportunityArticleEnhancements(
  applySearchConsoleArticleEnhancements(CORE_BASE_ARTICLES),
).map((article) =>
...
export const CORE_ARTICLES = publishedArticles(
  applyPublisherArticleReviewMetadata(ARTICLES_WITH_REVIEW_METADATA),
);
