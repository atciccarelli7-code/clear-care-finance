    ...
    <>
      <div className="print:hidden">
        ...
        <section className="border-b border-warning/25 bg-warning-soft/45 py-4" aria-labelledby="clinical-review-status">
          <div className="container flex max-w-5xl items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-relaxed text-muted-foreground">This is a complete source-checked educational preview, not a finalized clinical handout. It remains noindex and ad-free until a qualified independent reviewer verifies the condition-specific medication, monitoring, procedure, and emergency language.</p>
            </div>
          </div>
        </section>
        ...
        <div className="container max-w-5xl space-y-20 py-12 md:py-16">
          ...
          <GuideSection id="sources" eyebrow="Trust and verification" title="Sources used to build this guide" description="Professional guidance leads consequential claims; official patient resources support wording; major health sites are comparators."><div className="grid gap-6 lg:grid-cols-[1fr_0.7fr]"><SourceList sources={guide.sources} /><div className="space-y-4"><aside className="rounded-2xl border border-warning/30 bg-warning-soft/35 p-5 text-sm leading-relaxed text-muted-foreground"><div className="font-bold text-foreground">Clinical-review gate</div><p className="mt-2">This product will not be marked clinically final or opened to search indexing until an independent qualified reviewer is named and the claim-level review scope is recorded.</p></aside><Button asChild variant="outline" className="w-full justify-between"><a href={guide.benchmarkUrl} target="_blank" rel="noopener noreferrer">View the {guide.benchmarkName} <ExternalLink className="h-4 w-4" /></a></Button><Button asChild variant="soft" className="w-full justify-between"><Link to={DIAGNOSIS_EXPLAINED_ROUTE}>How this guide is governed <ArrowRight className="h-4 w-4" /></Link></Button></div></div><div className="mt-8"><DisclaimerBox /></div></GuideSection>
        </div>
      </div>

      <section className="hidden print:block print:p-0" aria-label={`Concise ${guide.diagnosisName} handout`}>
        <header className="border-b-2 border-black pb-3"><div className="text-xs font-bold uppercase tracking-wider">Community Acquired Finance · Clinical-review preview</div><h1 className="mt-2 text-3xl font-bold">{guide.shortTitle}: concise care handout</h1><p className="mt-2 text-sm">Source-checked educational preview. Independent clinical review is pending. The treating team’s written plan controls.</p></header>
