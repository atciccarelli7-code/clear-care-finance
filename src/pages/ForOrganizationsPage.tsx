...
import { ArrowRight, Building2, FileCheck2, LockKeyhole, ShieldCheck } from "lucide-react";
...
const ForOrganizationsPage = () => {
  useSeo({
    ...
    description: "Evaluate privacy-minimized Community Acquired Finance programs for workforce benefits, patient costs, Medicare and discharge, healthcare careers, and broad navigation.",
  ...
  return (
    <>
      <PageHero
        ...
        description="Evaluate the live participant experience, choose one focused program, and review the implementation and procurement boundaries before making a commitment."
      >
        <Button asChild variant="hero" size="lg"><a href="#program-builder">Build a program brief <ArrowRight className="h-4 w-4" /></a></Button>
        <Button asChild variant="outline" size="lg"><Link to="/for-organizations/trust-procurement" onClick={() => trackGrowthEvent("organization_trust_page_opened", { entry_surface: "organization", action_id: "hero" })}>Review trust & procurement</Link></Button>
      ...
      <section className="container max-w-7xl py-12 md:py-16" aria-labelledby="organization-fit-title">
      ...
      <section className="border-y border-border bg-card/25 py-14 md:py-20" aria-labelledby="programs-title">
        <div className="container max-w-7xl">
          <div className="mx-auto max-w-3xl text-center"><div className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Five program categories</div><h2 id="programs-title" className="mt-2 font-display text-3xl font-bold tracking-tight">Focused pathways built from products buyers can open today.</h2><p className="mt-3 leading-relaxed text-muted-foreground">A program adds approved launch materials, accountable roles, support, review, and measurement. It does not hide the participant product behind a sales demo.</p></div>
      ...
      <section className="container max-w-7xl py-14 md:py-20" aria-labelledby="difference-title">
        <div className="grid gap-5 lg:grid-cols-3">
          {[
            [LockKeyhole, "Privacy-minimized by design", "No participant account, identity file, PHI, plan document, diagnosis, claim number, or case narrative is required for the current public offer."],
        ...
        <div className="mt-10 overflow-hidden rounded-3xl border border-border bg-card shadow-card">
          <div className="grid md:grid-cols-3">
            {[
              ["1", "Evaluate", "Review live products, sources, privacy behavior, limitations, and buyer fit without a participant launch."],
