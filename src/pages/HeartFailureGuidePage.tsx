...
const sectionLinks = [
  ["#meaning", "Meaning"], ["#types", "Type"], ["#classification", "Stage & class"], ["#causes", "Causes"],
  ["#tests", "Tests"], ["#treatment", "Treatment"], ["#medications", "Medications"], ["#daily-plan", "Daily plan"],
  ["#action-plan", "When to get help"], ["#questions", "Questions"], ["#sources", "Sources"],
] as const;

const quickPaths = [
  { href: "#meaning", label: "Understand the diagnosis", detail: "Start with the 30-second explanation, type, stage, and functional class.", icon: HeartPulse },
  { href: "#medications", label: "Understand the medicines", detail: "Separate fluid relief from long-term heart protection and selected therapies.", icon: BookOpen },
  { href: "#daily-plan", label: "Know what to do each day", detail: "Review weight, symptoms, medicines, activity, and practical barriers.", icon: ListChecks },
  { href: "#action-plan", label: "Know when to get help", detail: "Separate emergencies from changes that need a prompt care-team call.", icon: AlertTriangle },
] as const;
    ...
    <>
      <div className="print:hidden">
        ...
        <section className="container max-w-5xl py-8">
          <div className="grid gap-4 md:grid-cols-2">
            <ContentFreshness lastReviewedAt="2026-07-23" nextReviewAt={guide.nextReviewAt} timeSensitive reviewScope="Adult heart-failure definition, ejection-fraction phenotype, stage, functional class, causes, diagnostic evaluation, treatment goals, medication purpose, devices, home monitoring, warning signs, and care-team questions." />
        ...
        <section id="quick-start" className="container max-w-5xl pb-8" aria-labelledby="quick-start-title">
          <div className="rounded-3xl border border-primary/20 bg-primary-soft/25 p-6 shadow-card md:p-8">
            <div className="max-w-2xl">
              ...
              <h2 id="quick-start-title" className="mt-2 font-display text-2xl font-bold md:text-3xl">Choose what you need right now</h2>
        ...
        <div className="container max-w-5xl space-y-20 py-12 md:py-16">
          ...
          <GuideSection id="types" eyebrow="Type or phenotype" title="The same diagnosis can describe different heart problems" description="Treatment makes more sense after the patient knows which broad ejection-fraction phenotype the care team is treating.">
          ...
          <GuideSection id="classification" eyebrow="Stage and functional class" title="Three labels can describe different parts of the same condition" description="Type, stage, and functional class answer different questions and should not be used interchangeably.">
            <div className="grid gap-5 lg:grid-cols-3">
              {guide.classificationSystems.map((item) => (
                <article key={item.name} className="rounded-3xl border border-border bg-card p-6">
                  <h3 className="font-display text-xl font-bold">{item.name}</h3>
                  <div className="mt-3 text-sm font-semibold text-secondary">Examples: {item.examples}</div>
                  <p className="mt-4 text-sm leading-relaxed text-muted-foreground"><strong className="text-foreground">Question answered: </strong>{item.questionAnswered}</p>
                </article>
              ))}
            </div>
            <div className="mt-6 rounded-2xl border border-warning/30 bg-warning-soft/35 p-5 text-sm leading-relaxed text-muted-foreground"><strong className="text-foreground">Keep the labels separate: </strong>{guide.classificationNote}</div>
          </GuideSection>
      ...
      <section className="hidden print:block print:p-0" aria-label="Concise heart failure handout">
        ...
        <div className="mt-5 grid grid-cols-2 gap-5 text-sm">
          ...
          <section>
            <h2 className="text-lg font-bold">Labels to confirm</h2>
            <ul className="mt-2 space-y-2">{guide.classificationSystems.map((item) => <li key={item.name}><strong>{item.name}:</strong> {item.examples}</li>)}</ul>
          </section>
          ...
          <section>
            ...
            <div className="mt-3"><strong>Contact the care team promptly:</strong><ul className="mt-1 list-disc pl-5">{guide.actionPlan[1].signs.slice(0, 5).map((sign) => <li key={sign}>{sign}</li>)}</ul></div>
          </section>
          ...
          <section className="col-span-2 border-t border-black pt-4">
            ...
            <p className="mt-4 font-semibold">Do not change medicine doses, sodium or fluid limits, or emergency thresholds from this handout. Use the patient-specific written plan.</p>
          </section>
        </div>
      </section>
    </>
