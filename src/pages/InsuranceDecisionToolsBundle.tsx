...
type ChecklistItem = { title: string; body: string };

const inputClass = "h-11 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground shadow-sm outline-none transition-smooth focus:border-primary focus:ring-2 focus:ring-primary/20";
const textareaClass = "min-h-24 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm text-foreground shadow-sm outline-none transition-smooth focus:border-primary focus:ring-2 focus:ring-primary/20";
...
const SourceBox = ({ sources }: { sources: { title: string; url: string }[] }) => (
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {[
        ...
        <Link key={href} to={href} className="rounded-2xl border border-border bg-card p-5 shadow-card transition-smooth hover:-translate-y-0.5 hover:shadow-hover">
          ...
          <div className="mt-2 flex items-center gap-2 text-xs font-semibold text-primary">Open tool <ArrowRight className="h-3.5 w-3.5" /></div>
        </Link>
      ))}
    </div>
  </section>
  ...
  const result = useMemo(() => {
    ...
    if (answers.limitedIncome) {
      return {
        ...
        verify: ["State Medicaid/Medicare Savings Program eligibility.", "Extra Help for Part D drug costs.", "Local SHIP counseling before enrollment."],
      };
    }
          ...
          <Card className="rounded-3xl border-primary/20 bg-primary-soft/30 shadow-card">
            ...
            <CardContent className="grid gap-5 md:grid-cols-2">
              ...
              <div className="md:col-span-2 flex flex-col gap-3 sm:flex-row">
                <Button asChild variant="hero"><Link to="/insurance/medicare-advantage">Compare sample plan cards</Link></Button>
...
export const PriorAuthorizationGuide = () => {
  ...
  const statusNotes: Record<string, ChecklistItem[]> = {
    pending: [
      { title: "Confirm receipt", body: "Ask whether the request is in the insurer's system and get a reference number." },
      { title: "Ask for the due date", body: "Ask the expected decision date and whether anything is missing." },
    ],
    denied: [
      { title: "Request the denial reason", body: "Ask for the exact denial language and appeal instructions." },
      { title: "Ask provider about appeal", body: "Ask whether the provider can submit clinical notes, peer-to-peer review, or an appeal." },
    ],
    missing: [
      { title: "Find the missing item", body: "Ask exactly what document, code, clinical note, or form is missing." },
      { title: "Confirm resubmission", body: "After the provider sends it, call back and confirm it was received." },
    ],
    urgent: [
      { title: "Ask about urgent review", body: "Ask what criteria are needed for expedited review and who can request it." },
      { title: "Document symptoms and timing", body: "Keep notes on worsening symptoms, dates, names, and reference numbers." },
    ],
  };
  ...
  return (
    <>
      ...
      <main className="container space-y-16 py-12 md:py-16">
          ...
          <Card className="rounded-3xl border-primary/20 bg-primary-soft/30 shadow-card">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {statusNotes[status].map((item) => <div key={item.title} className="rounded-xl border border-border bg-card p-3"><h3 className="text-sm font-bold">{item.title}</h3><p className="mt-1 text-sm text-muted-foreground">{item.body}</p></div>)}
              </div>
        ...
        <section>
          <SectionHeading centered eyebrow="Checklist" title="What to document" />
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {["Date and time", "Person you spoke with", "Reference number", "Decision deadline", "Missing information", "Appeal or peer-to-peer option", "Urgent review criteria", "Next follow-up date"].map((item) => <div key={item} className="rounded-2xl border border-border bg-card p-4 text-sm font-semibold shadow-card"><CheckCircle2 className="mb-2 h-4 w-4 text-emerald-600" />{item}</div>)}
          </div>
        </section>
        <ToolLinkGrid />
  ...
  return (
    <>
      ...
      <main className="container space-y-16 py-12 md:py-16">
        ...
        <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <Card className="rounded-3xl shadow-card"><CardHeader><CardTitle className="font-display text-2xl">Build your discharge checklist</CardTitle></CardHeader><CardContent className="space-y-4">
          ...
          <Card className="rounded-3xl border-primary/20 bg-primary-soft/30 shadow-card"><CardHeader><CardTitle className="font-display text-2xl">Questions to ask</CardTitle><CardDescription>Use this with case management, insurance, and the receiving provider.</CardDescription></CardHeader><CardContent><ul className="space-y-3 text-sm text-muted-foreground">{checklist.map((item) => <li key={item} className="flex gap-2"><ClipboardCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{item}</li>)}</ul><Button className="mt-5" type="button" variant="outline" onClick={printPage}><Printer className="h-4 w-4" /> Print/save</Button></CardContent></Card>
        </section>
...
export const MedicationCoverageChecklist = () => {
  ...
  useSeo({ title: "Medication Coverage Checklist", description: "A structured medication coverage checklist for checking formulary, tiers, pharmacy network, prior authorization, step therapy, and quantity limits before choosing a health plan.", canonicalPath: "/insurance/medication-coverage-checklist" });
  ...
  return (
    <>
      ...
      <main className="container space-y-16 py-12 md:py-16"><AdSlot placement="after-hero" /><SoftWarning title="This is not a live drug database" body="Use this checklist to organize what to verify. Confirm live coverage in the plan formulary, Medicare Plan Finder, insurer tools, and pharmacy pricing before enrolling." />
        <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]"><Card className="rounded-3xl shadow-card"><CardHeader><CardTitle className="font-display text-2xl">Medication details</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        </CardContent></Card><Card className="rounded-3xl border-primary/20 bg-primary-soft/30 shadow-card"><CardHeader><CardTitle className="font-display text-2xl">What to verify</CardTitle><CardDescription>{medication} · {dose} · {pharmacy}{cost ? ` · ${cost}/month` : ""}</CardDescription></CardHeader><CardContent className="space-y-5"><ul className="space-y-3 text-sm text-muted-foreground">{["Exact medication name, dose, and formulation.", "Formulary tier and whether generic substitution applies.", "Preferred pharmacy, standard pharmacy, and mail-order pricing.", "Prior authorization, step therapy, quantity limits, and refill timing.", "What changes if the plan year changes."].map((item) => <li key={item} className="flex gap-2"><Pill className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{item}</li>)}</ul>{redFlags.length > 0 && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><h3 className="font-bold text-amber-950">Red flags</h3><ul className="mt-2 space-y-2 text-sm text-amber-950/80">{redFlags.map((flag) => <li key={flag}>{flag}</li>)}</ul></div>}<div className="flex flex-col gap-3 sm:flex-row"><Button asChild variant="hero"><Link to="/insurance/medicare-advantage">Back to plan comparison</Link></Button><Button asChild variant="outline"><Link to="/open-enrollment">Open Enrollment guide</Link></Button></div></CardContent></Card></section>
...
export const MedicalBillReviewToolkit = () => {
  ...
  useSeo({ title: "Medical Bill Review Toolkit", description: "A patient-friendly medical bill review toolkit with itemized bill, EOB, financial assistance, insurance processing, and appeal call scripts.", canonicalPath: "/insurance/medical-bill-review-toolkit" });
  ...
  return (
    <>
      ...
      <main className="container space-y-16 py-12 md:py-16"><AdSlot placement="after-hero" /><SoftWarning title="Do not ignore bills, but do not assume the first number is final" body="For large balances, confirm the EOB, itemized bill, insurance processing, network status, and financial assistance options before paying when possible." />
        <section className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]"><Card className="rounded-3xl shadow-card"><CardHeader><CardTitle className="font-display text-2xl">Bill review inputs</CardTitle></CardHeader><CardContent className="space-y-4"><label className="space-y-2"><span className="text-sm font-semibold">Bill amount</span><input className={inputClass} value={bill} onChange={(event) => setBill(event.target.value)} /></label>{[["eob", "Received an EOB?"], ["match", "Bill matches EOB?"], ["network", "Provider in-network?"], ["itemized", "Requested itemized bill?"], ["assistance", "Asked about financial assistance?"], ["collections", "Bill in collections?"]].map(([id, label]) => <label key={id} className="space-y-2"><span className="text-sm font-semibold">{label}</span><select className={inputClass} value={answers[id]} onChange={(event) => setAnswers((current) => ({ ...current, [id]: event.target.value }))}><option value="unsure">Unsure</option><option value="yes">Yes</option><option value="no">No</option></select></label>)}</CardContent></Card><Card className="rounded-3xl border-primary/20 bg-primary-soft/30 shadow-card"><CardHeader><CardTitle className="font-display text-2xl">Next steps</CardTitle></CardHeader><CardContent className="space-y-5"><ul className="space-y-3 text-sm text-muted-foreground">{nextSteps.map((step) => <li key={step} className="flex gap-2"><Receipt className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{step}</li>)}</ul><div className="rounded-2xl border border-primary/20 bg-card p-4 text-sm leading-relaxed">{billingScript}</div><Button type="button" variant="outline" onClick={printPage}><Printer className="h-4 w-4" /> Print/save</Button></CardContent></Card></section>
...
export const HealthcareWorkerPaycheckTools = () => {
  useSeo({ title: "Healthcare Worker Paycheck Tools", description: "A hub of paycheck, overtime, retirement, savings, and benefit tools for nurses and healthcare workers.", canonicalPath: "/healthcare-workers/paycheck-tools" });
  const tools = [
    ...
    ["Roth vs Traditional", "/tools", "Post-AdSense tool candidate if pending PR is merged."],
    ["Shift differential calculator", "/tools", "Coming soon scaffold for night/weekend differential math."],
    ["Can I afford to max my 403(b)?", "/tools", "Coming soon scaffold for savings-rate and paycheck tradeoffs."],
  ];
  return <><PageHero eyebrow="Healthcare worker money" title="Healthcare Worker Paycheck Tools" description="Your paycheck is the engine. Benefits, overtime, differentials, tax accounts, and retirement elections all change what actually lands in your bank account." /><main className="container space-y-16 py-12 md:py-16"><AdSlot placement="after-hero" /><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{tools.map(([title, href, body]) => <Link key={title} to={href} className="rounded-3xl border border-border bg-card p-6 shadow-card transition-smooth hover:-translate-y-0.5 hover:shadow-hover"><WalletCards className="mb-4 h-6 w-6 text-primary" /><h2 className="font-display text-xl font-bold">{title}</h2><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{body}</p><div className="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-primary">Open</div></Link>)}</div></main></>;
};

export const MedicareAdvantageVsMedigap = () => {
  useSeo({ title: "Medicare Advantage vs Medigap", description: "A plain-English comparison of Medicare Advantage and Original Medicare plus Medigap tradeoffs.", canonicalPath: "/insurance/medicare-advantage-vs-medigap" });
  const rows = [
  ...
  return <><PageHero eyebrow="Medicare comparison" title="Medicare Advantage vs Medigap" description="The real tradeoff is usually lower premium and extra benefits versus provider flexibility, prior authorization, and supplemental coverage cost." /><main className="container space-y-16 py-12 md:py-16"><AdSlot placement="after-hero" /><SoftWarning title="Do not treat either option as universally best" body="The better fit depends on doctors, hospitals, prescriptions, travel, income, Medigap enrollment timing, state rules, and risk tolerance." /><div className="overflow-hidden rounded-3xl border border-border bg-card shadow-card"><div className="overflow-x-auto"><table className="min-w-[820px] w-full text-left text-sm"><thead><tr className="border-b border-border bg-muted/40"><th className="p-4">Question</th><th className="p-4">Medicare Advantage</th><th className="p-4">Original Medicare + Medigap</th></tr></thead><tbody>{rows.map(([label, ma, medigap]) => <tr key={label} className="border-b border-border/70 last:border-0"><th className="bg-muted/20 p-4 align-top font-semibold">{label}</th><td className="p-4 align-top text-muted-foreground">{ma}</td><td className="p-4 align-top text-muted-foreground">{medigap}</td></tr>)}</tbody></table></div></div><ToolLinkGrid /><SourceBox sources={[{ title: "Medicare.gov — Compare Original Medicare and Medicare Advantage", url: "https://www.medicare.gov/basics/get-started-with-medicare/get-more-coverage/your-coverage-options/compare-original-medicare-medicare-advantage" }, { title: "Medicare.gov — Medigap", url: "https://www.medicare.gov/health-drug-plans/medigap" }]} /></main></>;
};

export const InsuranceMarketingRealityPage = () => {
  useSeo({ title: "What Medicare Advantage Ads May Not Emphasize", description: "A balanced, practical article on what Medicare Advantage marketing may highlight and what patients still need to verify.", canonicalPath: "/insurance/what-medicare-advantage-ads-may-not-emphasize" });
  const items = [
  ...
  return <><PageHero eyebrow="Insurance marketing" title="What Medicare Advantage Ads May Not Emphasize" description="Marketing often highlights benefits. Patients still need to verify the less exciting details before choosing a plan." /><main className="container space-y-16 py-12 md:py-16"><AdSlot placement="after-hero" /><div className="grid gap-5 md:grid-cols-2">{items.map(([title, body]) => <Card key={title} className="rounded-3xl shadow-card"><CardHeader><Badge tone="amber">Check this</Badge><CardTitle className="font-display text-2xl">{title}</CardTitle><CardDescription className="text-base leading-relaxed">{body}</CardDescription></CardHeader></Card>)}</div><Accordion type="single" collapsible className="rounded-3xl border border-border bg-card px-5 shadow-card"><AccordionItem value="balanced"><AccordionTrigger className="text-left font-display text-xl font-bold">Balanced take</AccordionTrigger><AccordionContent><p className="text-sm leading-relaxed text-muted-foreground">Medicare Advantage can be a reasonable fit for some people. The point is not to avoid it automatically. The point is to compare the plan based on the care you might need in a bad year, not only the benefits that look attractive during a healthy year.</p></AccordionContent></AccordionItem></Accordion><ToolLinkGrid /></main></>;
};

export const InsuranceDecisionToolsIndex = () => {
  useSeo({ title: "Insurance Decision Tools", description: "Clear Care Finance insurance decision tools for Medicare Advantage, prior authorization, discharge coverage, medication coverage, and medical bills.", canonicalPath: "/insurance" });
  return <><PageHero eyebrow="Insurance tools" title="Insurance Decision Tools" description="Patient- and caregiver-friendly tools for the coverage decisions that usually get confusing when care is expensive or time-sensitive." /><main className="container space-y-16 py-12 md:py-16"><AdSlot placement="after-hero" /><ToolLinkGrid /></main></>;
};
