...
const PatientsFamilies = () => <>
  ...
  <section className="container pt-10 md:pt-14">
    <SectionHeading centered eyebrow="Diagnosis, Explained" title="Bring the diagnosis. Leave with a clearer mental model." description="Nine complete, source-checked, nurse-reviewed guides are available. They do not diagnose symptoms, collect health details, or direct medication changes." />
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      <article className="overflow-hidden rounded-3xl border border-primary/20 bg-gradient-to-br from-primary-soft/75 via-card to-card p-6 shadow-card md:p-8">
        ...
        <div className="mt-5 text-xs font-bold uppercase tracking-[0.16em] text-primary">Published guide</div>
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-muted-foreground"><span className="rounded-full border border-border bg-background/80 px-3 py-1.5">Purpose-first medicines</span><span className="rounded-full border border-border bg-background/80 px-3 py-1.5">Three-level action plan</span><span className="rounded-full border border-primary/25 bg-primary-soft/60 px-3 py-1.5">Nurse reviewed</span></div>
        <Button asChild variant="accent" size="lg" className="mt-6 w-full sm:w-auto"><Link to={HEART_FAILURE_GUIDE_ROUTE} onClick={() => trackSiteEvent("patient_guide_resource_selected", { item_id: "heart_failure_guide_preview", stage_id: "understand_diagnosis", destination_path: HEART_FAILURE_GUIDE_ROUTE })}>Open heart-failure guide <ArrowRight className="h-4 w-4" /></Link></Button>
      </article>
      ...
      <article className="overflow-hidden rounded-3xl border border-secondary/20 bg-gradient-to-br from-secondary-soft/65 via-card to-card p-6 shadow-card md:p-8">
        ...
        <div className="mt-5 text-xs font-bold uppercase tracking-[0.16em] text-secondary">Published guide</div>
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-muted-foreground"><span className="rounded-full border border-border bg-background/80 px-3 py-1.5">Inhaler technique</span><span className="rounded-full border border-border bg-background/80 px-3 py-1.5">Oxygen safety</span><span className="rounded-full border border-primary/25 bg-primary-soft/60 px-3 py-1.5">Nurse reviewed</span></div>
        <Button asChild variant="accent" size="lg" className="mt-6 w-full sm:w-auto"><Link to={COPD_GUIDE_ROUTE} onClick={() => trackSiteEvent("patient_guide_resource_selected", { item_id: "copd_guide_preview", stage_id: "understand_diagnosis", destination_path: COPD_GUIDE_ROUTE })}>Open COPD guide <ArrowRight className="h-4 w-4" /></Link></Button>
      </article>
    </div>
    ...
    <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {ADDITIONAL_DIAGNOSIS_GUIDES.map((guide) => (
        <article key={guide.slug} className="flex flex-col rounded-3xl border border-border bg-card p-6 shadow-card">
          ...
          <div className="mt-4 text-xs font-bold uppercase tracking-[0.16em] text-primary">Published guide</div>
          ...
          <Button asChild variant="outline" className="mt-5 w-full justify-between"><Link to={guide.route} onClick={() => trackSiteEvent("patient_guide_resource_selected", { item_id: `${guide.slug}_guide_preview`, stage_id: "understand_diagnosis", destination_path: guide.route })}>Open guide <ArrowRight className="h-4 w-4" /></Link></Button>
        </article>
      ))}
    </div>
