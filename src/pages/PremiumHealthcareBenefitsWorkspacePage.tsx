...
export default function PremiumHealthcareBenefitsWorkspacePage() {
  ...
  if (loading) return <main className="grid min-h-[70vh] place-items-center bg-[#f4f6f2]"><div className="flex items-center gap-3 rounded-2xl border border-border bg-white px-5 py-4 text-sm font-semibold shadow-card" role="status"><Loader2 className="h-5 w-5 animate-spin text-primary" /> Loading secure workspace…</div></main>;
  if (!payload || !progress || !product) return <main className="bg-[#f4f6f2] py-20"><div className="container max-w-2xl"><div className="rounded-3xl border border-border bg-white p-8 text-center shadow-card"><LockKeyhole className="mx-auto h-10 w-10 text-primary" /><h1 className="mt-5 font-display text-3xl font-bold">Workspace unavailable</h1><p className="mt-3 text-muted-foreground">{error || "The protected product could not be loaded."}</p><div className="mt-6 flex justify-center gap-3"><Button onClick={() => void loadWorkspace()}><RefreshCw className="h-4 w-4" /> Retry</Button><Button asChild variant="outline"><Link to="/premium/access">Access page</Link></Button></div></div></div></main>;

  return (
    <main className="min-h-screen bg-[#f4f6f2] text-[#183326]">
      <style>{`@media print{header,footer,.premium-no-print{display:none!important}.premium-shell{display:block!important}.premium-print-surface{box-shadow:none!important;border:0!important;padding:0!important}.premium-print-only{display:block!important}.premium-screen-only{display:none!important}body{background:#fff!important}.premium-module-page{break-before:page}.premium-module-page:first-child{break-before:auto}a{color:#183326!important;text-decoration:none!important}}@media screen{.premium-print-only{display:none!important}}`}</style>
