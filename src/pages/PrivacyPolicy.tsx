  ...
  return (
    <>
      ...
      <section className="container max-w-3xl py-12 md:py-16 space-y-6" aria-label="Privacy policy details">
        <p className="text-sm text-muted-foreground">Effective date: August 10, 2026</p>
        ...
        <Section title="Premium purchases, customer accounts, and entitlements">
          <p>Paid commerce is not currently active. If later authorized, checkout will be processed by Stripe Checkout. Community Acquired Finance will not intentionally receive or store full payment-card numbers. Stripe may provide customer, checkout-session, payment-intent, payment-status, test-mode, refund, and other transaction information needed to verify a purchase and control access.</p>
          <p>The intended account provider is Supabase Auth using email magic links. Supabase manages authentication sessions; Community Acquired Finance server functions validate the session token before checking product entitlement. Reaching a checkout return page, changing browser storage, or setting a client-side flag does not grant access.</p>
          <p>The intended account database stores the profile, product entitlement, workspace title, completion state, validated workspace answers, assumptions, and final user-selected decision. It is not designed to store uploads, payment-card data, bank information, insurance member identifiers, medical records, diagnoses, claim documents, paystubs, full financial statements, protected health information, or confidential employer documents.</p>
          <p>A development-only local demo may store non-production demo state in that browser when an explicit local flag is enabled. The production build rejects that flag. Browser printing creates a separate local output controlled by the user.</p>
        </Section>

        <Section title="Data deletion and reset">
          <p>The production workspace foundation supports deletion of a user-owned workspace. Account deletion and any required transaction-record retention procedure must be finalized and tested before paid access opens. Deleting a workspace does not automatically delete a transaction record or revoke product access because certain records may be needed to process refunds, prevent fraud, and meet accounting or legal obligations.</p>
        ...
        <Section title="Information collected automatically">
          ...
          <p>After a visitor chooses Allow analytics, limited first-party evidence systems may also store a random event identifier, a random browser-session identifier, an allowlisted lifecycle event, a fixed site surface and journey identifier, an optional fixed phase or experiment version, a bounded step number, and a server timestamp in Supabase. The browser-session identifier is stored in session storage and is not linked by these systems to an account, name, email address, form answer, calculator input, health detail, financial amount, URL, query string, referrer, IP address, or device fingerprint. This evidence is used to count whether a bounded site pathway or guided decision was viewed, started, advanced, completed, copied, printed, resumed, restarted, or handed off to a predefined next step.</p>
        </Section>
        ...
        <Section title="Privacy choices and Google Consent Mode">
          ...
          <p>Necessary only keeps Google analytics storage, advertising storage, advertising user data, and advertising personalization denied, and the site does not download the Google Analytics library or send site-generated first-party evidence events. Allow analytics grants Google Analytics storage, loads Google Analytics, and permits the limited first-party evidence events described above, while advertising storage, advertising user data, and advertising personalization remain denied through this site&apos;s consent controls.</p>
          <p>Site-generated analytics event URLs are reduced to an origin and pathname before they enter the analytics data layer. Query strings and URL fragments are not intentionally included in page-view, destination-path, or outbound-link event fields. First-party evidence systems store fixed destination or journey identifiers rather than URLs.</p>
          <p>Necessary hosting, transaction, account-access, security, and diagnostic records may continue because they are required to operate and protect the service. The site does not intentionally send guided-tool answers, employer or role names, salary or wage amounts, benefit amounts, loan balances, principal, APRs, payments, terms, fees, lender names, quote details, calculated outcomes, commute details, names, email addresses, income, health status, free-text notes, or other sensitive form entries as analytics or first-party evidence properties.</p>
        </Section>
